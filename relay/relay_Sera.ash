...
"if ($('#showall').is(':checked')) $('.unavail').show(400); else $('.unavail').hide(400); return false; }); });</script>");
...
foreach c,l,rec in sorted {
   ...
   if (rec.note != "") {
      ...
      if (c == "fight" && rec.note.to_monster() != $monster[none]) {
         page.append("<br>");
         for i from 1 to 3 page.append(i > fs[normalized(rec.note,"monster")] ? " <span class='dimmed'>&bull;</span> " : " <big>&bull;</big> ");
      }
