...
{
  if (!SIM_CONSUME)
    return have_effect($effect["got milk"]);
  return 100000;
}
...
float ode_adjust(int drunkness, float adv)
{
	return adv+min(have_effect($effect["ode to booze"]),drunkness);
...
float milk_adjust(int fullness, float adv)
{
	return adv+min(have_effect($effect["got milk"]),fullness);
...
//worth it.
  ...
  while (shouldgetmilk && get_milk()<fullness && (milkprice != lastprice)) 
  {
    if (SIM_CONSUME)
    {
...
float value(con_rec con, boolean overdrink)
{
  ...
  {
    milk = 0;
    ode = have_effect($effect["ode to booze"]);
  }
  ...
  {
    if (milk!=0)
    { advscore = (milk_adjust(full,averange(con.adv)) 
		  * VALUE_OF_ADVENTURE) / full; }
    else if (ode!=0)
    { advscore = (ode_adjust(full,averange(con.adv)) 
		  * VALUE_OF_ADVENTURE) / full; }
    else
...
void filter_have()
{
  foreach i in grub
  {
    if (grub[i].have < 1)
    {
      verbose2(grub[i].it+" is not in inventory and we can't buy more.");
      remove grub[i];
    }
  }
}
...
boolean getone(item it, int price)
{
  ...
  if (!gotit && MAKE && (creatable_amount(it) >= 1 || (true_have_bartender() && my_adventures()==0))) //creatable amount is 0 when no adventures since mafia is retarded

  {
  cli_execute("refresh campground");
    if (!true_have_chef() && GET_CHEF)
    {
    ...
    if (!true_have_bartender() && GET_BARTENDER)
    {
    ...
    if (pause > 0)
      wait(pause);
	if(it==$item[cherry bomb] || it==$item[sangria del diablo]  || it==$item[vesper]  || it==$item[dirty martini]  || it==$item[bodyslam]  || it==$item[grogtini] )
	{
		print("making tps drink","red");
		item mix_fruit;
		item skewered_fruit;
		item mix_booze;
		item improved_booze;
		if(it==$item[cherry bomb])
		{
			mix_fruit=$item[cherry];
			skewered_fruit=$item[skewered cherry];
			mix_booze=$item[bottle of whiskey];
			improved_booze=$item[old-fashioned];
		}
		else if(it==$item[sangria del diablo])
		{
			mix_fruit=$item[cherry];
			skewered_fruit=$item[skewered cherry];
			mix_booze=$item[boxed wine];
			improved_booze=$item[sangria];
		}
		else if(it==$item[grogtini])
		{
			mix_fruit=$item[lime];
			skewered_fruit=$item[skewered lime];
			mix_booze=$item[bottle of rum];
			improved_booze=$item[grog];
		}
		else if(it==$item[bodyslam])
		{
			mix_fruit=$item[lime];
			skewered_fruit=$item[skewered lime];
			mix_booze=$item[bottle of tequila];
			improved_booze=$item[tequila with training wheels];
		}
		else if(it==$item[dirty martini])
		{
			mix_fruit=$item[jumbo olive];
			skewered_fruit=$item[skewered olive];
			mix_booze=$item[bottle of gin];
			improved_booze=$item[dry martini];
		}
		else if(it==$item[vesper])
		{
			mix_fruit=$item[jumbo olive];
			skewered_fruit=$item[skewered olive];
			mix_booze=$item[bottle of vodka];
			improved_booze=$item[dry vodka martini];
		}
		//now make it
		if(item_amount(mix_fruit)<2)
		{
			buy(2-item_amount(mix_fruit),mix_fruit);
		}
		if(item_amount(improved_booze)<1)
		{
			if(item_amount(mix_booze)<1)
			{
				buy(1,mix_booze);
			}
		}
		//make skewered fruit
		print("craft.php?mode=cocktail&pwd&action=craft&a=938&b="+ to_int(mix_fruit)+"&qty=1&master=Shake%21","red");
		visit_url("craft.php?mode=cocktail&pwd&action=craft&a=938&b="+to_int(mix_fruit)+"&qty=1&master=Shake%21");
		//make improved booze
		if(item_amount(improved_booze)<1)
		{
			print("craft.php?mode=cocktail&pwd&action=craft&a="+to_int(mix_fruit)+"&b="+to_int(mix_booze)+"&qty=1&master=Shake%21","red");
			visit_url("craft.php?mode=cocktail&pwd&action=craft&a="+to_int(mix_fruit)+"&b="+to_int(mix_booze)+"&qty=1&master=Shake%21");
		}
		//make tps drink
		print("craft.php?mode=cocktail&pwd&action=craft&a="+to_int(skewered_fruit)+"&b="+to_int(improved_booze)+"&qty=1&master=Shake%21","red");
		visit_url("craft.php?mode=cocktail&pwd&action=craft&a="+to_int(skewered_fruit)+"&b="+to_int(improved_booze)+"&qty=1&master=Shake%21");
	}
	else
	{
...
boolean consumeone(con_rec con, string type)
{
  ...
  if ((item_amount(con.it) > 0) || SIM_CONSUME)
  {
    ...
    if (!SIM_CONSUME)
    {
      ...
      if (type == "food")
	  {
		//if we don't actually have one already make sure we have a chef first
		boolean success=true;
		if(item_amount(con.it)<1)
		{
			success=true_have_chef();
		}
		if(success)
		{
      ...
      if (type == "drink")
      {
		print("drinking "+con.it,"red");
		//if we don't actually have one already make sure we have a bartender first
		boolean success=true;
		if(item_amount(con.it)<1)
		{
			success=true_have_bartender();
		}
		if(success)
		{
			// Lets see if we want to get some ode via a mood!
			if (have_effect($effect[ode to booze]) == 0 && wants_ode == true)
			{
			...
			{//check for flute use
				if(ode_adjust(averange(con.consumptionGain),averange(con.adv))>25 && item_amount($item[Divine champagne flute])>0)
				{
					cli_execute("mood execute");
	...
	simadventures = simadventures + milk_adjust(averange(con.consumptionGain),averange(con.adv));
      }
      if (type == "drink")
      {
	siminebriety = siminebriety + con.consumptionGain.min;
	simadventures = simadventures + ode_adjust(averange(con.consumptionGain),averange(con.adv));
      }
      if (type == "spleen")
      {
...
//  if ((item_amount($item["tiny plastic sword"]) > 0) && !true_have_bartender())
...
////////// 4 is overdrink
  for iteration from 1 upto 4 by 1
  {
	...
	summarize("At drunkenness of "+doneness(type)+". Overdrinking.");
	  ...
	  //if can't interact, or first char under level 5, filter by what we have
	  if(!can_interact() || (!in_moxie_sign() && !in_muscle_sign() && !in_mysticality_sign() && !in_bad_moon() && my_level()<5))



	  {
		print("filtering out items you don't have","blue");
		filter_have();
	  }
	verbose("Choosing "+type+" to consume.");
	filter_final(max, doneness(type));
	int to_consume = best();  
	if (my_inebriety()>inebriety_limit()) //or if we are overdrinking while already overdrunk
	{
	  verbose("No "+type+" fits inside you. Moving on.");
	  giveup = true;
	}
	if (to_consume == 0) //or if we are overdrinking while already overdrunk

	{
	  verbose("No "+type+" available that's good enough.");
	  giveup = true;
	}
	else
	{
  ...
  if (get_milk()!=0 || (have_effect($effect["ode to booze"]) > 0))
  {
