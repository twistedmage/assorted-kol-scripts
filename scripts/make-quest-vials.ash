...
if ( inv[RED] < 6 )
{
	abort( "You need at least 6 vials of red slime" );
}

inv[BLUE] = item_amount( $item[vial of blue slime] );
if ( inv[BLUE] < 6 )
{
	abort( "You need at least 6 vials of blue slime" );
}

inv[YELLOW] = item_amount( $item[vial of yellow slime] );
if ( inv[YELLOW] < 6 )
{
	abort( "You need at least 6 vials of yellow slime" );
}
...
mix(2, ORANGE, RED, YELLOW);
mix(2, GREEN, YELLOW, BLUE);
mix(2, VIOLET, RED, BLUE);
