#/******************************************************************************
#                     First Things First 3.9 by Zarqon
...
float monstervalue() {
   ...
   foreach num,rec in item_drops_array() {
      switch (rec.type) {
        // pickpocket-only
         case "p": if (my_primestat() != $stat[moxie] || stolen != $item[none]) break; should_pp = true;
        // filter conditional drops
         case "c": if (item_type(rec.drop) == "shirt" && !have_skill($skill[torso awaregness])) break; // skip shirts if applic
            if (!is_displayable(rec.drop)) break;                          // skip quest items
                                                                           // include the rest????
        // normal drops
         case "": if (stolen == $item[none] && rec.rate*(item_drop_modifier()+100)/100.0 < 100) should_pp = true;
         case "n": if (rec.drop == stolen && !skipped) { skipped = true; break; }
            res = res + item_val(rec.drop,rec.rate);
      }
   }
   return res;
}
...
void set_autoputtifaction() {
  ...
  // next, handle effective goal-getting (this only olfacts if you set "goals" for autoOlfact)
   ...
   boolean[monster] sources;
   foreach m in menage if (has_goal(menage[m]))
      sources[menage[m]] = true;
   if (count(sources) == 0) return;
   print(count(sources)+"/"+count(menage)+" monsters dropping goals.");
   if (sources contains last_monster()) {
      if (count(sources) == 1) {
         vprint("This monster drops all your remaining goals!","green",3);
      ...
      } else vprint("This monster drops at least one of your goals.","olive",3);
   }
}
...
string use_special_item(string page) {    // mostly doesn't support funkslinging
   if (flyers != $item[none] && to_boolean(vars["flyereverything"]) && my_location() != $location[battlefield (hippy uniform)] &&
       my_location() != $location[battlefield (frat uniform)] && my_location() != $location[haunted bathroom] && my_location() != $location[primordial soup])
   ...
   string flyerboss(item special) {       // handles flyering / mirror/basepair-ing Cy and GMoB (supports funkslinging)
      if (flyers != $item[none]) {
         if (item_amount(special) > 0) {
            ...
            if (contains_text(page," your entangling noodles") ||
                user_confirm("You have both a(n) "+special+" and "+flyers+", but can't funksling them.  Throw flyers?"))
               act(throw_item(flyers));
  ...
  // ID spheres transparent with mafia's autoSphereID
   if (my_level() > 10 && get_property("autoSphereID") == "true" && intheclear() && vprint("Checking for unidentified spheres...",9))
...
string ftf(string page) {
   ...
   if (!intheclear() || (to_boolean(vars["flyereverything"]) && flyers != $item[none]) ||
...
check_version("First Things First","FTF","3.9",1255);
