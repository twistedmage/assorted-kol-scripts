...
// If true, you won't receive the "Are you sure you want to..." message on 
// eat without milk.
setvar("eatdrink_suppressNoMilk", true);
...
boolean EAT_SHOP = to_boolean(vars["eatdrink_shop"]);
...
if (have_skill($skill[ode to booze]) == false)
{ wants_ode = false; }

// If you want to invoke "ode to booze" we will do it via a mood
...
setvar("eatdrink_avoid_"+replace_string(to_string($item[wet stew])," ","_"), 
       true);
...
cli_execute("zlib eatdrink_piePriority = true");
...
cli_execute("zlib eatdrink_priceOfNontradeables = 0");
...
boolean get_on_budget(int totalquantity, item it, int maxprice) 
{
   ...
   if (startmeat > budget)
   {
     ...
     verbose("You now have a total of "+item_amount(it));
	 ...
	 verbose("Seen a problem with this one before at a price of "
       ...
       verbose("Setting new effective price to "+BUDGET);
...
int effective_price(con_rec con)
{
  ...
  //you should have fresh prices.
    if(historical_age(it) > to_float(vars["eatdrink_maxAge"]))
    {
      price = mall_price(it);
...
con_rec availability(con_rec con)
{
  ...
  if (EAT_MAKE && (con.it != $item["digital key lime pie"])) 
  {
...
print("item="+con.it);
  ...
  //prevent div by 0 for stupid items
  if(full==0)
	return 0;
  float advscore = averange(con.adv)*VALUE_OF_ADVENTURE / full;
  float statscore = averange(con.muscle)*musclescore/full + averange(con.mysticality)*mysticalityscore/full + averange(con.moxie)*moxiescore/full;
  float costscore = (con.price) / to_float(full);
  ...
  if (overdrink)
  {
	//if we are overdrinking remove the division by fullness as all fit just as well
    advscore = advscore * full;
    statscore = statscore * full;
    costscore = costscore * full;
	//also for overdrink consider ode
	int ode_turns = min(full,have_effect($effect[ode to booze]));
	advscore = advscore + ode_turns*VALUE_OF_ADVENTURE;
  }
...
void filter_type(string type)
{
  foreach i in grub
  {
  print("type check "+grub[i].it+": type="+grub[i].type);
    if (grub[i].type != type)
    {
      print(grub[i].it+" isn't a "+type+". Skipping.");
      remove grub[i];
    }
  }
}

//added by simon, remove things we don't have, for use in ronin/hc
void filter_have()
{
  foreach i in grub
  {
  print("have check "+grub[i].it+": have="+grub[i].have);
    if (grub[i].have < 1)
    {
      print(grub[i].it+" is not in inventory and we can't buy more.");
      remove grub[i];
    }
  }
}
...
void filter_level()
{
  ...
  foreach i in grub
  {
  print("level check "+grub[i].it+": level="+grub[i].level);
    if (grub[i].level > level)
    {
		print(grub[i].it+" has a minimum required level of "
...
void set_prices()
{
  foreach i in grub
  {
	...
	verbose("Favorite "+grub[i].it+" appears unavailable given budget, "
	      + "EAT_SHOP variable settings, ronin status, and/or mall price.");
      }
      remove grub[i];
    }
  }
...
boolean special_purchases(item it)
{
  ...
  if (it == $item[bodyslam])
  {
    ...
    get_on_budget(1, $item[bottle of tequila], 
		  mall_price($item[bottle of tequila]) * PRICE_FLEXIBILITY);
    special = create(1, it);
  }
  else if (it == $item[cherry bomb])
  {
    ...
    get_on_budget(1, $item[bottle of whiskey], 
		  mall_price($item[bottle of whiskey]) * PRICE_FLEXIBILITY);
    special = create(1, it);  
  }
  else if (it == $item[dirty martini])
  {
    get_on_budget(2, $item[jumbo olive], 
		  mall_price($item[jumbo olive]) * PRICE_FLEXIBILITY);
    get_on_budget(1, $item[bottle of gin], 
		  mall_price($item[bottle of gin]) * PRICE_FLEXIBILITY);
    special = create(1, it); 
  }
  if (it == $item[grogtini])
  {
    get_on_budget(2, $item[lime], mall_price($item[lime]) * PRICE_FLEXIBILITY);
    get_on_budget(1, $item[bottle of rum], 
		  mall_price($item[bottle of rum]) * PRICE_FLEXIBILITY);
    special = create(1, it);  
  }
  ...
  else if (it == $item[vesper])
  {
    get_on_budget(2, $item[jumbo olive], 
		  mall_price($item[jumbo olive]) * PRICE_FLEXIBILITY);
    get_on_budget(1, $item[bottle of vodka], 
		  mall_price($item[bottle of vodka]) * PRICE_FLEXIBILITY);
    special = create(1, it);
  }
  else if (it == $item[jarlsberg's key lime pie] || 
	   it == $item[boris's key lime pie] || 
	   it == $item[sneaky pete's key lime pie])
  {
    get_on_budget(1, $item[pie crust], 
		  mall_price($item[pie crust]) * PRICE_FLEXIBILITY);
...
void filter_final(int max, int current)
{
  foreach i in grub
  {
    if (fail[grub[i].it])
	{
		print("removing "+grub[i].it+" because it's true in fail","red");
		remove grub[i];
    }
	else
	{
		if (!inbudget(grub[i]))
		{
			print("removing "+grub[i].it+" because it's out of budget","red");
		  remove grub[i];
		}
		else if (grub[i].consumptionGain.min > (max - current))
		{
		  print(to_string(grub[i].it)+" is too fattening ("
...
boolean getone(item it, int price)
{
	print("fail flag for "+it+" is set to "+fail[it]+" at start","blue");
  if (SIM_CONSUME)
  {
  ...
  if (!gotit && EAT_MAKE && (creatable_amount(it) >= 1))
  {
	print("creating one "+it,"blue");
	cli_execute("refresh campground");
    if (!simons_have_chef() && GET_CHEF)
    {
		simons_get_chef();
    }
    if (!simons_have_bartender() && GET_BARTENDER)
    {
  ...
  if (!gotit && !EAT_SHOP)
  {
    ...
    //foreach i in grub { if (grub[i].it == it) remove grub[i]; }
  }
  if (!gotit && (price == 0))
  {
    summarize("EatDrink encountered an error: You don't have a "+it
  ...
  if (!gotit)
  {
	print("failed to get "+it,"blue");
	fail[it] = true;
  }
  else
  {
	print("got an "+it,"blue");
  }
	print("fail flag for "+it+" is set to "+fail[it]+" at end","blue");
  return gotit;
}
...
boolean consumeone(con_rec con, string type)
{
  ...
  print("finished getone, now have "+item_amount_before,"blue");
  if ((item_amount(con.it) > 0) || SIM_CONSUME)
  {
		if (type == "food")
		{
			print("eating food","blue");
		...
		if (type == "drink")
		{
			...
			{//check for flute use
				if(averange(con.consumptionGain)>25 && item_amount($item[Divine champagne flute])>0)
				{
					cli_execute("mood execute");
					cli_execute("burn extra");
					print("TRYING"+"inv_booze.php?utensil=3123&which=1&whichitem="+to_int(con.it)+"&pwd","blue");
					visit_url("inv_booze.php?utensil=3123&which=1&whichitem="+to_int(con.it)+"&pwd");
				}
				else
					overdrink(1, con.it);
			}
			...
			{//check for flute use
				if(averange(con.consumptionGain)>25 && item_amount($item[Divine champagne flute])>0)
				{
					cli_execute("mood execute");
					cli_execute("burn extra");
					print("TRYING"+"inv_booze.php?utensil=3123&which=1&whichitem="+con.it+"&pwd","blue");
					visit_url("inv_booze.php?utensil=3123&which=1&whichitem="+con.it+"&pwd");
				}
				else
					drink(1, con.it);
			}
    ...
    else 
    {
		if (type == "food")
		{
			...
			simadventures = simadventures + averange(con.consumptionGain);
		}
		if (type == "drink")
		{
...
//return value says if we should recall it
...
//    verbose("WARNING: You have a tiny plastic sword but no bartender, so the effective cost of TPS drinks will include the price of the bartender, making them less attractive.  If you want to drink TPS drinks, you should probably buy a bartender-in-the-box before running this.","RED");
  ...
  if (!CONSIDER_COST_WHEN_OWNED)
    begin = begin + "not";
  begin = begin+" be a factor if you own it already. Hagnk's pulls "
    +"(if enabled) will cost ";
  begin = begin+COST_OF_PULL+" meat each.";
  summarize(begin);
   begin = "An adventure has the value of <b>"+VALUE_OF_ADVENTURE
    +" meat</b>. "+my_primestat()+" subpoint is "+VALUE_OF_PRIME_STAT;
...
////////// 4 is overdrink
  for iteration from 1 upto 4 by 1
  {
    ...
    if (can_do && (doneness(type) < conmax))
    {
      ...
      if (iteration != 4)
	summarize(type+": At "+doneness(type)+", consuming to "+conmax+".");
      else
	summarize("At drunkenness of "+doneness(type)+". Overdrinking.");
      ...
      while (!(giveup || (doneness(type) >= conmax) || 
	       ((doneness(type) > inebriety_limit()) && (iteration == 4)))) 
      {
		verbose("Choosing "+type+" to consume.");
		filter_final(conmax, doneness(type));
		int to_consume = best();  
		if (to_consume == 0) //or if we are overdrinking while already overdrunk
		{
		...
		else
		{
		  ...
		  else
		  {
			...
			int oldprice = grub[to_consume].price;
			grub[to_consume].price = effective_price(grub[to_consume]);
			verbose(grub[to_consume].it+" price updated from "+oldprice+" to "
				+ grub[to_consume].price+".");
			foreach g in grub // update new values (milk, ode in effect)
			{
			  grub[g].value = value(grub[g], (iteration == 4)); 
			}	
			print_html("FAIL: "+consume_entry);
			logprint("FAIL: "+consume_entry);
	      ...
	      int advmeat, int primemeat, int offmeat, int pullmeat,boolean sim)
{
  ...
  SIM_CONSUME = sim;
  //use milk as needed
  int milk_needed = (foodMax - my_fullness())-have_effect($effect[got milk]);
  while(milk_needed>0)
  {
		use(1,$item[milk of magnesium]);
		milk_needed = (foodMax - my_fullness())-have_effect($effect[got milk]);
  }
  int ode_needed = (drinkMax - my_inebriety())-have_effect($effect[ode to booze]);
  while(ode_needed>0)
  {
		cli_execute("cast ode");
		ode_needed = (foodmax - my_fullness())-have_effect($effect[ode to booze]);
  }
  eatdrink(foodMax, drinkMax, spleenMax, overdrink);
}
