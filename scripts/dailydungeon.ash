#/******************************************************************************
#                    Daily Dungeon Diver 1.9 by Zarqon
#*******************************************************************************
#
  ...
  // little function that mafia can't do
   ...
   for i from 1 to 9 {
      ...
      if (p[i].contains_text("Locked Door")) {
         ...
         if (item_amount($item[skeleton key]) + creatable_amount($item[skeleton key]) > 2) {
            vprint("Using skeleton key: +1 key","gray",2); keys += 1; continue;
         }
...
boolean fetch_roomdata() {
   string url = replace_string(visit_url("http://www.noblesse-oblige.org/calendar/daily_"+today_to_string()+".html"),"heat","hot");
...
check_version("Daily Dungeon Diver","DDD","1.9",1866);
void main() {
