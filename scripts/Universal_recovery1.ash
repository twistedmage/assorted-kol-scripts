// Bale's all purpose Universal recovery script v 3.71
...
// If an integer property is undefined, this function returns 1.
int default_one(string pref) {
	if(!is_integer(pref)) return 1;
	return pref.to_int();
}

// These properties are set by the relay browser UI:
   // Verbosity: 0=none, 1=normal, 2=verbose, 3=chatterbox
int Verbosity = get_property("baleUr_Verbosity").default_one();
   // Is it allowable to purchase from the mall or NPCS?
int baleUr_Purchase = get_property("baleUr_Purchase").default_one();
...
boolean mallcore = can_interact() && buy_mall;
...
void restore_values() {
	...
	heal[null] = new restorative_info(0, 0, 0, 0, 0.0, 0.0, false);
	if(have_equipped($item[Dyspepsi-Cola-issue canteen])) { // This item doubles cola effectiveness
		heal[$item[Cloaca-Cola]] = new restorative_info(0, 0, 20, 28, 0.0, 24.0, false);
		heal[$item[Dyspepsi-Cola]] = new restorative_info(0, 0, 20, 28, 0.0, 24.0, false);
	}
	if(to_item("potion of healing") != $item[none]) 	// This is the Dungeon of Doom healing potion.
		heal[to_item("potion of healing")] = new restorative_info(4, 16, 4, 16, 10.0, 10.0, false);
	...
	heal[$item[Palm frond]] = new restorative_info(18, 22, 18, 22, 20.0, 20.0, false);
	if(mallcore || contains_text(hpAutoRecoveryItems, "really thick bandage"))
		heal[$item[mummy wrapping]] = new restorative_info(50, 60, 0, 0, 55.0, 0.0, false);
	if(mallcore)  // This is only handled freely in mallcore mode
		heal[$item[six-pack of New Cloaca-Cola]] = new restorative_info(0, 0, 840, 960, 0.0, 900.0, false);
...
void shadow_prep() {
	if(have_skill($skill[Ambidextrous Funkslinging])) {
		int stock = available_amount($item[gauze garter]) + available_amount($item[gauze garter]);
		if(stock <= 6) {
			gg = item_amount($item[gauze garter]);
			fp = item_amount($item[filthy poultice]);
		} else {
			stock = 6 - closet_amount($item[gauze garter]) - closet_amount($item[filthy poultice]);
			if(stock <0) stock = 0;
...
// Can I access the Knob Laboratory Store? Will I lose Hp/Mp for switching into the Elite Guard Uniform?
boolean seltzer_available() {
	if(have_outfit("knob goblin elite guard uniform") && item_amount($item[Cobb's Knob lab key]) == 1) {
		cli_execute("whatif equip Knob Goblin elite helm; equip Knob Goblin elite polearm; equip Knob Goblin elite pants; quiet");
		int guard_hp = numeric_modifier("_spec", "Buffed HP Maximum");
		int guard_mp = numeric_modifier("_spec", "Buffed MP Maximum");
		if(guard_mp < my_mp() || (guard_hp < my_hp() && (start_type == "HP" || guard_hp < hp_autoheal)))
			return false;
		return true;
	}
	return false;
...
item [slot] mp_gear(int target) {
	...
	if(have_effect($effect[Dreams and Lights]) > 0)
	...
	boolean test_gear(slot where, item it) {
		...
		if(have_effect($effect[Beaten Up]) > 0) {  	// If beaten up, this is evaluating tongue skills.
...
void populate_skills(int target) {
	clear(mp_gear);
...
// How many times do I need to cast the skill?
float cast_quant(skill it, int target){
	if(it == $skill[none] || skills[it].mp > my_maxmp()) return 0;
	float q = to_float(target - my_hp())/ skills[it].ave;
	q = min(q, floor(my_maxmp() / skills[it].mp));
	return max(q, 1.0);
}
...
item best_mall(int amount, string type) {
	...
	//	if(historical_age(key) > 1.5) 	// Ensure that historical_price() is good
	//		mall_price(key);
		...
		case $item[plump juicy grub]:
			if(have_effect($effect[Form of...Bird]) < 1)
				continue;		// Only use moths and grubs if currently in birdform.
			break;
		case $item[Medicinal Herb's medicinal herbs]:
			if(!use_herb || my_spleen_use() >= spleen_limit())
				continue;		// Don't use medicinal herbs unless specified in options.
			cost = q * 100;
			break;
		case $item[magical mystery juice]:
			if(!buy_mmj && !(item_amount($item[magical mystery juice]) > 1 && UseMmjStock))
				continue;		// Make certain mmj can be purchased.
			cost = q * 100;
			break;
		...
		if(Verbosity > 2) print("Could heal "+amount+type+" with "+to_string(q.ceil())+" "+key+" for "+cost+" meat.", "Purple");
	...
	if(type == "MP") {
		if(best_item == $item[magical mystery juice])
			cost = 100.0;
		else cost = item_price(best_item);
...
boolean use_mall(int q, item it){
	int ihave;
	switch(it) {
	case $item[mummy wrapping]:
	case $item[Palm frond]:
	...
	else if(heal[it].avehp > 0 && heal[it].avemp == 0 && have_effect($effect[Beaten Up]) == 0)
		set_property("_meatperhp", to_string(historical_price(it)/ heal[it].avehp));
	use_inv(min(q, for_use(it)), it);
	return true;
}
...
boolean mall_heal(int target, string type) {
	...
	repeat {
		...
		if(type == "HP" && best_skill != $skill[none] && q_skill > 0
		  && mp_to_meat(q_skill * skills[best_skill].mp) <= q_item * historical_price(best_item)) {
		...
		} else if(!use_mall(q_item, best_item)) {
...
string meatpermp() {
	...
	case seltzer_available():
		return to_string(80.0/ heal[$item[knob seltzer]].avemp);
...
void meatper() {
	...
	if(can_interact()) {
		...
		item_mph = meatperhp();
	if(skill_mph < item_mph)
		set_property("_meatperhp", to_string(max(skill_mph, 0.001)));
	else
		set_property("_meatperhp", to_string(max(item_mph, 0.001)));
}
...
boolean visit_nuns(int target, string type) {
	...
	if(type == "HP" && hpAutoRecoveryItems.contains_text("visit the nuns") && get_property("sidequestNunsCompleted") == "hippy") {
		if(my_maxhp() < 1600 && my_hp() < my_maxhp() /8.0) {
			go_nuns(1);
...
item [int] choose_inventory(string type) {
	...
	if(type == "HP") {				// This will collect the HP type restoratives in inventory.
		...
		foreach key, value in heal
			...
			case $item[plump juicy grub]: 
				if(have_effect($effect[Form of...Bird]) > 0 && item_amount(key) > 0)
...
int max_heal(item [int] options) {
	...
	foreach key, it in options
		...
		default:
			if(it == reserve) 
				total_heal = total_heal + healwith_this(it, 1);
			else total_heal = total_heal + healwith_this(it, 0);
		}
	return total_heal;
}
...
boolean inv_mp_restore(int target) {
	...
	if(have_effect($effect[Form of...Bird]) > 0 && item_amount($item[Delicious shimmering moth]) > 0 
...
boolean purchase_mp(int target) {
	...
	repeat {
		...
		if(buy_mmj && my_meat() >= 100) {
			...
			else if(seltzer_available())
				best_buy = $item[knob seltzer];
		...
		if(best_buy != null) {
			...
			if(best_buy == $item[knob seltzer])		// If best is seltzer, purchase extra for next time to save outfit switches.
				purchase(min(max((my_meat() - (q * 80))/ 320, 0), 2 * q) + q, $item[knob seltzer]);
			else purchase(q, best_buy); 	// If we don't have anything to use, then we're done.
			q = min(q, for_use(best_buy));
			use(q, best_buy);
		} else if(fizzy) {
			// Let's not heal all the way to mp_autotarget if not required -- expensive!
			if(start_type == "MP") {
				if(objective == 0) target = min(target *.9, hp_autoheal);
				else target = objective;  // In the case that objective !=0, target is lowered to original minimum
			}
...
boolean mp_heal(int target){
	...
	if(item_amount($item[Yendorian Express])>0 && get_property("expressCardUsed") == "false"
	  && my_mp() < (my_maxmp() /9) && contains_text(mpAutoRecoveryItems, "platinum yendorian express card"))
		use(1, $item[Yendorian Express]); 	// Yendorian Express Card is only used if it'll give OODLES of mp!
	...
	if(have_effect($effect[Form of...Bird]) > 0 && !mallcore && item_amount($item[Delicious shimmering moth]) > 0 
	  && heal[$item[Delicious shimmering moth]].avemp* BirdThreshold + my_mp() <= my_maxmp())
  		use(inv_quant($item[Delicious shimmering moth], target- my_mp(), "MP").max(1), $item[Delicious shimmering moth]);
	if(contains_text(mpAutoRecoveryItems, "free disco rest") && numeric_modifier("Base Resting MP") >= 10)
	...
	if(contains_text(mpAutoRecoveryItems, "sleep on your clan sofa") && my_level()*5 > rest_mp
	  && contains_text(visit_url("clan_rumpus.php"), "otherimages/clanhall/rump5_3.gif")) {
		...
		while(my_mp() < target && my_maxmp() - my_mp() >=rest_mp && my_maxhp() - my_hp() >= rest_hp)
...
boolean fullheal() {
	...
	string select_method() {
		switch {
		case have_effect($effect[Form of...Bird]) > 0 && item_amount($item[plump juicy grub]) > 0 
		  && heal[$item[plump juicy grub]].avehp + my_hp() >= my_maxhp():
		...
		case use_herb && my_spleen_use() < spleen_limit()
		  && (item_amount($item[medicinal herbs]) >0 || (my_meat() >=100 && my_primestat() == $stat[muscle] && buy_npc)):
			if(item_amount($item[medicinal herbs]) < 1)
...
boolean inv_hp_restore(int target, item [int] options) {
	...
	if(have_effect($effect[Form of...Bird]) > 0 && item_amount($item[plump juicy grub]) > 0 
...
boolean cheapest_beatup() {
	...
	case "Tongue of the Walrus":
		...
		break;
	default:
		use_mall(1, best.to_item());
	}
	if(have_effect( $effect[Beaten Up] )<1) {			// If cured, then maxhp and maxmp have changed!
		set_autohealing();
		return true;
	}
	print("Unable to cure beaten up! Go sewer or adventure someplace wussier.", "red");
	return false;
}
...
boolean cure_beatenup(int target){
	...
	if(have_effect($effect[Beaten Up])>0 && item_amount($item[Soft green echo eyedrop antidote])>1 
...
boolean hp_heal(int target){
	...
	if(!mallcore && have_effect($effect[Form of...Bird]) > 0 && item_amount($item[plump juicy grub]) > 0 
	  && heal[$item[plump juicy grub]].avehp + my_hp() >= my_maxhp() 
	  && heal[$item[plump juicy grub]].avehp * BirdThreshold + my_hp() <= my_maxhp()) {
	...
	//	if(can_interact() && (5-hotTubSoaks)*1000 + my_hp() < target) return false; 	// Since a full heal will be used anyway...
	...
	if(my_hp() >= target || (use_hot_tub() && hot_tub()) || visit_nuns(target, "HP"))
	...
	if(contains_text(hpAutoRecoveryItems, "free disco rest") && numeric_modifier("Base Resting MP") >= 10)
	...
	if(skill_restore(target)) 			// Let's try to restore HP by casting skills.
		return true;
	// Should I use the Clan Sofa? Only hit the server to look for the sofa if preferences say to rest there.
	if(contains_text(hpAutoRecoveryItems, "sleep on your clan sofa") && my_level()*5 > rest_hp
	  && contains_text(visit_url("clan_rumpus.php"), "otherimages/clanhall/rump5_3.gif")) {
		if((my_maxmp() - my_mp()) >=my_level()*5 && (my_maxhp() - my_hp()) >= my_level()*5) {
		...
		while(my_hp() < target && (my_maxmp() - my_mp()) >=rest_mp && (my_maxhp() - my_hp()) >= rest_hp)
			cli_execute("rest");			// This will waste adventures resting if set in HP/MP Usage.
...
boolean unpoison() {
	// Am I poisoned?
	boolean poisoned() {
		foreach poison in $effects[Hardly Poisoned at All, A Little Bit Poisoned, Really Quite Poisoned,
		  Somewhat Poisoned, Majorly Poisoned]
			if(have_effect(poison)>0) return true;
		return false;
	}
	// Won't purchase antidotes if not allowed to purchase from NPC stores.
	void acquire_antidote(int howmany) {
		if(howmany > item_amount($item[anti-anti-antidote]) && buy_npc)
	...
	if(my_level() > 4) 	// There's no reason to stockpile antidotes until level 5, minimum
		switch {
...
void choose_reserve_purch() {
	if(mallcore) {
		...
		foreach key, choice in heal
			if(is_tradeable(key) && choice.combat && choice.minmp > 20) {
		...
		if (buy_mmj) {
			reserve_purch = $item[magical mystery juice];
		} else if (have_outfit("knob goblin elite guard uniform") && item_amount($item[Cobb's Knob lab key]) > 0)
			reserve_purch = $item[knob seltzer];	// Be careful in case guard uniform lowers max hp & mp
		switch {
...
void reserve_healing() {
	...
	else if (reserve_purch == $item[knob seltzer] && my_meat() < 80 * (reserve_num-purch_have))
		reserve_num = floor(reserve_num /80.0);
	if(purch_have < reserve_num && reserve_purch != null) {
		switch (reserve_purch) {
		case $item[knob seltzer]:
			if(!seltzer_available())
				break;
		case $item[magical mystery juice]:
		default:
...
void daily_handling() {
	...
	if(current_ver != thisver) {
		...
		if(count(heal) == 0 || (curr != "" && get_property(fname+".txt") != curr)) 
			get_newmap(curr);
...
// Let's start restoring. This will cure poisoning, cure beaten up and finally heal HP or MP. Returns false if cannot fully heal.
boolean restore(string type, int amount) {
	...
	restore_values();
	daily_handling(); 	// This will do something on the first run of the day
	reserve_healing();
	if(type == "MP") {
		if(objective == 0 && mp_autoheal < 0) return true;
		if(have_effect($effect[Beaten Up])>0 && objective == 0 && hp_autoheal >= 0)
			cure_beatenup(objective);
		// Check to see if the player needs MP for auto-Olfaction.
		if(get_property("autoOlfact") != "" && have_effect($effect[On the Trail]) < 1
	    ...
	    if((objective != 0 && my_hp() < objective) || (objective == 0 && start_hp<= hp_autoheal && my_hp() < amount))
