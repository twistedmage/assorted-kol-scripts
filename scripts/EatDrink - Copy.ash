...
{
  return (max(1, item_amount($item[seal-skull helmet])) + max(1, item_amount($item[seal-clubbing club])) + max(1, item_amount($item[helmet turtle])) + max(1, item_amount($item[turtle totem])) + max(1, item_amount($item[ravioli hat])) + max(1, item_amount($item[pasta spoon])) + max(1, item_amount($item[hollandaise helmet])) + max(1, item_amount($item[saucepan])) + max(1, item_amount($item[disco mask])) + max(1, item_amount($item[disco ball])) + max(1, item_amount($item[mariachi hat])) + max(1, item_amount($item[stolen accordion])) + max(1, item_amount($item[old sweatpants])));
}
...
{
  int price = 0;
  if (!is_tradeable(it) && npc_price(it) == 0 && is_displayable(it))
  {
...
boolean have_ingredients(item newitem, int count, int [item] stack, int loop)
{
  ...
  foreach i in stack
  {
    ...
    else if (EAT_COINMASTER && i == $item[jabanero pepper] ||
        i == $item[ketchup] || i == $item[catsup])
    {
      int worthless = $coinmaster[hermit].available_tokens;
      if (!to_boolean(get_property("hermitHax0red")))
      {
        worthless = min(worthless, floor(get_meat(1) / 100));
        makeusedmeat += worthless * 100;
      }
      if (stack[i] < 0 && worthless > 0)
        stack[i] += worthless;
      int hermitcost = (14 - get_starter_items()) * 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100);
      while (stack[i] < 0 && get_meat(1) >= hermitcost && EAT_SHOP)
      {
        stack[i] += 1;
        makeusedmeat += hermitcost;
        hermitcost = 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100);
      }
    }
...
int [item] add_ingredients(item newitem, int count, int [item] stack, int loop)
{
  ...
  foreach i in working
  {
    ...
    else
    {
      ...
      if (EAT_COINMASTER && (i == $item[jabanero pepper] ||
          i == $item[ketchup] || i == $item[catsup]))
      {
        int worthless = $coinmaster[hermit].available_tokens;
        if (!to_boolean(get_property("hermitHax0red")))
          worthless = min(worthless, floor(get_meat(1) / 100));
        if (working[i] < 0 && worthless > 0)
        {
          stack[i] += (working[i] * -1) < worthless ? working[i] : (worthless * -1);
          working[i] += worthless;
        }
        int hermitcost = (14 - get_starter_items()) * 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100);
        while (working[i] < 0 && get_meat(1) >= hermitcost && EAT_SHOP)
        {
          stack[i] -= 1;
          working[i] += 1;
          makeveryusedmeat += hermitcost;
          hermitcost = 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100);
        }
      }
      int npcprice = npc_price(i);
...
boolean collect_ingredients(item newitem, int count, int [item] stack, int loop)
{
  ...
  foreach i in working
  {
    ...
    if (working[i] > 0 && EAT_COINMASTER &&
        (i == $item[jabanero pepper] || i == $item[ketchup] || i == $item[catsup]))
    {
      if (working[i] > 0 && $coinmaster[hermit].available_tokens > 0)
        while (hermit(1, i) && working[i] > 0) working[i] -= 1;
      if (working[i] > 0 && get_meat(1) >= (14 - get_starter_items()) * 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100) && EAT_SHOP)
      {
        retrieve_item(working[i], $item[worthless item]);
        if ($coinmaster[hermit].available_tokens > 0 && hermit(working[i], i))
          working[i] = 0;
      }
    }
    int npcprice = npc_price(i);
...
boolean collect_hermit_ingredients(item newitem, int count, int [item] stack, int loop)
{
  int [item] working;
  ...
  foreach i in working
  {
        ...
        (i == $item[jabanero pepper] || i == $item[ketchup] || i == $item[catsup]))
    {
      if (working[i] > 0 && $coinmaster[hermit].available_tokens > 0)
        while (hermit(1, i) && working[i] > 0) working[i] -= 1;
      while (working[i] > 0 && get_meat(1) >= (14 - get_starter_items()) * 50 + (to_boolean(get_property("hermitHax0red")) ? 0 : 100) && EAT_SHOP)
      {
        retrieve_item(working[i], $item[worthless item]);
        if ($coinmaster[hermit].available_tokens > 0 && hermit(working[i], i))
          working[i] = 0;
    ...
    while (working[i] > 0 && EAT_MALL && !get_ronin() && npcprice == 0 && get_meat(2) >= effective)
      working[i] -= 1;
    if (working[i] > 0 && EAT_MAKE && count(get_ingredients(i)) > 0 && collect_hermit_ingredients(i, working[i], working, loop+1))
      working[i] = 0;
    if (working[i] > 0)
      return false;
  }
...
void set_prices()
{
  foreach i in grub
  {
    ...
    if (grub[i].have < 1 && !grub[i].mustShop && !grub[i].mustMall) remove grub[i];
  }
}
...
//  output += " make:"+entry.mustMall;
//  output += " make:"+entry.mustShop;
//  output += " make:"+entry.canShop;
...
float value(con_rec con, boolean overdrink)
{
  ...
  if(con.it==$item[green drunki bear] || con.it==$item[red drunki bear] || con.it==$item[yellow drunki bear])
	full=8; //pretend it takes 8 drunkenness instead of 4 of each
  float advscore = 0;
  float statscore = 0;
  float costscore = 0;
...
void filter_final(int max, int current)
{
  foreach i in grub
  {
    ...
    else
    {
      ...
      else if (grub[i].have < 1 && !(EAT_SHOP && grub[i].canShop) && !(EAT_MALL && !get_ronin() && grub[i].mustMall))
        remove grub[i];
    }
  }
}
...
boolean stackone(con_rec con, string type, boolean speculative)
{
  ...
  if (!con.mustMake && !con.mustShop && !con.mustMall) con.using += 1;
  if (con.mustPull || con.mustShop || con.mustMall) stepusedmeat += con.price;
}
...
//////////Print out the "starting" message that contains your config choices
  ...
  {
    vprint("You're in ronin, and not simulating, so no shopping for you.",3);
    EAT_MALL = false;
  }
...
////////// 4 is overdrink
  while (iteration < 4)
  {
    ...
    if (can_do && (doneness(type) < conmax))
    {
      ...
      while ((ORIGINAL_VOA / 2) < VALUE_OF_ADVENTURE && ORIGINAL_VOA > 200)
      {
        VALUE_OF_ADVENTURE -= 100;
      ...
      giveup = false;
      //each pass, use the a stack and try to collect items for consuming.  Aborts when you giveup.
#      if (!SIM_CONSUME && !collect_hermit_ingredients($item[none], 0, ingredients, 0)) { giveup = true; repeatingstep = true; }
      ...
      while (!(giveup || (nomposition >= maxposition)))
      {
        ...
        else
        {
          ...
          if ((iteration == 3) && (get_spleen() > 5) && !triedmojo && !mojogiveup)
          {
            ...
            if (checkspleen != get_spleen()) repeatingstep = true;
